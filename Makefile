...
HTTPS_GIT := github.com/gnodi-network/gnodi/v2.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=gnodi \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=gnodid \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/gnodid ./cmd/gnodid
endif

build-linux: 
	GOOS=linux GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/gnodid-linux ./cmd/gnodid

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/gnodid.exe ./cmd/gnodid
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/gnodid
...
draw-deps:
	...
	@goviz -i ./cmd/gnodid -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c gnodi --local -f chains.yaml
endif
...
set-testnet-configs:
	gnodid config set client chain-id gnodi
	gnodid config set client keyring-backend test
	gnodid config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | gnodid keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | gnodid keys add acc1 --recover`

testnet: setup-testnet
	spawn local-ic start testnet

sh-testnet: mod-tidy
	CHAIN_ID="gnodi" BLOCK_TIME="1000ms" CLEAN=true sh scripts/test_node.sh

sh-testnet-multi-node: mod-tidy
	CHAIN_ID="gnodi" BLOCK_TIME="1000ms" CLEAN=true sh scripts/test_2node.sh
